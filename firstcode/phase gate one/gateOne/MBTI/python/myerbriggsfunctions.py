...
def get_first_test_result(response):
	...
	the_reesponses = getresponse(response)
	
	number_of_As = 0
	number_of_Bs = 0

	for count in range(0, 17, 4):
		if the_reesponses[count] == 'A':
			number_of_As += 1
		else:
			number_of_Bs += 1
	if number_of_As > number_of_Bs:
		return 'E'
	else:
		return 'I'	

def get_second_test_result(response):
...
def get_third_test_result(response):
...
def get_fourth_test_result(response):
...
def personality(response):
	...
	message = personality_messages()
	first_response = get_first_test_result(response)
	second_response = get_second_test_result(response)
	third_response = get_third_test_result(response)
	fourth_response = get_fourth_test_result(response)
	
	personality_test = first_response + second_response + third_response + fourth_response;
		
	if personality_test in "ISTJ":
		return message[0]

	elif personality_test in "INFJ":
		return message[1]

	elif personality_test in "INTJ":
		return message[2]

	elif personality_test in "ENFJ":
		return message[3]

	elif personality_test in "ISTP":
		return message[4]

	elif personality_test in "ESFJ": 
		return message[5]

	elif personality_test in "INFP": 
		return message[6]

	elif personality_test in "ESFP": 
		return message[7]

	elif personality_test in "ENFP": 
		return message[8]

	elif personality_test in "ESTP":
		return message[9]

	elif personality_test in "ESTJ": 
		return message[10]

	elif personality_test in "ENTJ": 
		return message[11]

	elif personality_test in "INTP": 
		return message[12]

	elif personality_test in "ISFJ":
		return message[13]

	elif personality_test in "ENTP":
		return message[14]
	else:
		return message[15];
